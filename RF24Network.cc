...
void RF24Network::begin(uint8_t _channel, uint16_t _node_address,rf24_datarate_e data_rate, rf24_pa_dbm_e pa_level)
{
  ...
  while ( radio.available(&pipe_num) )
  {
   // printf("radio aviable %d\r\n",pipe_num);
    ...
    while (!done)
    {
      ...
      IF_SERIAL_DEBUG(printf_P(PSTR("%u: MAC Received on %u %s\n\r"),millis(),pipe_num,header->toString()));
      IF_SERIAL_DEBUG(const uint16_t* i = reinterpret_cast<const uint16_t*>(frame_buffer + sizeof(RF24NetworkHeader));
        printf_P(PSTR("%u: NET message %04x\n\r"),millis(),*i)
      );
...
bool RF24Network::enqueue(void)
{
  ...
  IF_SERIAL_DEBUG(printf_P(PSTR("%u: NET Enqueue @%lx "),millis(),next_frame-frame_queue));
...
const char* RF24NetworkHeader::toString(void) const
{
  ...
  //sprintf(buffer,sizeof(buffer),PSTR("id %04x from 0%o to 0%o type %c"),id,from_node,to_node,type);
  return buffer;
}
...
uint64_t pipe_address( uint16_t node, uint8_t pipe )
{
  ...
  printf_P(PSTR("%u: NET Pipe %i on node 0%o has address %x%x\n\r"),millis(),pipe,node,*top,*out));
